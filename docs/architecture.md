...
| Sprint 15 | Chantier LUKS2 + NBDE (Tang/Clevis) terminé avec succès sur matériel réel | Disque root chiffré, déverrouillage automatique via serveur Tang (réseau statique dans hook initramfs custom, DHCP indisponible à ce stade). Voir `infra/disk-encryption/` pour la solution complète documentée |
| Sprint 15 | Remise en état post-chiffrement : partition agrandie (growpart+cryptsetup resize+resize2fs, 4G->29G), Docker réinstallé, SSH/GHCR reconfigurés, SSH sécurisé (clé uniquement) | Voir `infra/disk-encryption/POST-FLASH-SETUP.md` pour la procédure complète |
| Sprint 15 | Story 8 (AIPD) : sauvegarde chiffrée automatisable validée (Pi → export Docker + GPG local -> Mac via scp sortant, jamais de connexion entrante sur le Mac) | Test de restauration complet réussi (déchiffrement + extraction), confirmant l'exploitabilité réelle, pas juste la création de fichiers |
| Sprint 15 | Story 8 (AIPD) automatisée : sauvegarde chiffrée quotidienne via launchd (3h du matin), déclenchement Pi→Mac inversé (Mac initie SSH sortant, jamais de connexion entrante exposée) | Test de restauration validé. Limitation connue non bloquante : avertissement SSH "host identification changed" persiste dans les logs launchd malgré known_hosts nettoyé manuellement — la sauvegarde aboutit quand même (5 fichiers .gpg créés avec succès malgré l'avertissement), cause probable liée au contexte réseau isolé de launchd résolvant différemment le mDNS |
| Sprint 15 | Nouveaux endpoints API pour le futur dashboard React : GET /identities (liste sans données biométriques), DELETE /identities/{id}, GET /stats | Préparation du frontend investisseur. list_identities() exclut volontairement les vecteurs d'embedding par design, jamais exposés via cette route |
| Sprint 16 | Dashboard React/Next.js complet et validé : 4 pages (dashboard, enrôlement, identités, journal), identification en direct dans le flux vidéo avec cooldown de journalisation (30s), capture d'enrôlement depuis la caméra du Pi (pas la webcam du navigateur) | Testé de bout en bout avec succès. Prêt pour démo investisseur |
